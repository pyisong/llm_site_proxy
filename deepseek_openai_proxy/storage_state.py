...
def storage_state_login_issue(state: dict[str, Any]) -> str | None:
    session_id = extract_ds_session_id(state)
    if not session_id:
        return (
            "deepseek_storage.json 缺少 ds_session_id cookie。"
            "请重新运行 python3 -m save_storage_state 导出登录态。"
        )
    ...
    return (
        "deepseek_storage.json 缺少 Bearer 登录令牌（localStorage.userToken 为空且未捕获 authorization）。"
        "请在聊天页发送一条测试消息并收到回复后，再按 Enter 导出；"
        "或在 DevTools → Network 中查看 completion 请求的 authorization 头是否非空。"
    )
...
def merge_cookie_lists(*groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for cookies in groups:
        for cookie in cookies:
            ...
            domain = str(cookie.get("domain") or ".deepseek.com")
...
def storage_state_summary(state: dict[str, Any]) -> str:
    ...
    return (
        f"ds_session_id={'有' if session_id else '无'}, "
