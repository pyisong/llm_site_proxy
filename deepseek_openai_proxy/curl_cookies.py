...
def parse_curl_cookies(curl_text: str, *, domain: str = ".deepseek.com") -> tuple[list[dict[str, object]], str | None]:
...
def load_auth_from_text(text: str, *, domain: str = ".deepseek.com") -> tuple[list[dict[str, object]], str | None]:
    stripped = text.strip()
...
def load_auth_from_file(path: str | Path, *, domain: str = ".deepseek.com") -> tuple[list[dict[str, object]], str | None]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="从 curl 命令解析 DeepSeek cookie")
    parser.add_argument("input", nargs="?", help="curl 文件路径；省略则从 stdin 读取")
    parser.add_argument("-o", "--output", default="secrets/deepseek_cookies.json", help="输出 JSON 路径")
    parser.add_argument("--domain", default=".deepseek.com", help="cookie domain")
