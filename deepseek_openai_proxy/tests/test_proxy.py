...
async def test_models_requires_local_key():
    app = create_app(local_api_key="local-secret", deepseek_api_key="upstream-secret", backend_mode="official")
...
@pytest.mark.anyio
async def test_chat_completion_forwards_openai_payload_to_deepseek():
...
@pytest.mark.anyio
async def test_browser_backend_uses_logged_in_browser_without_deepseek_api_key():
    ...
    assert body["model"] == "deepseek-chat-web"
...
async def test_browser_backend_new_chat_header_overrides_default():
    ...
    transport = httpx.ASGITransport(app=app)
    headers = {**auth_headers(), "X-DeepSeek-New-Chat": "true"}
...
async def test_chat_completion_logs_request_and_response(caplog):
    ...
    with caplog.at_level(logging.INFO, logger="deepseek_openai_proxy"):
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                "/v1/chat/completions",
                headers=auth_headers(),
