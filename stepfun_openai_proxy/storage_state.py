...
def extract_session_cookie(state: dict[str, Any]) -> str | None:
    auth = state.get("stepfun_auth")
    if isinstance(auth, dict):
    ...
    for cookie in state.get("cookies") or []:
        if isinstance(cookie, dict) and cookie.get("name") in {"session_id", "stepfun_session"}:
...
def storage_state_login_issue(state: dict[str, Any]) -> str | None:
    ...
    if cookies or origins:
        return None
    return "stepfun_storage.json 未包含 cookies 或 origins，请重新运行 python3 -m save_storage_state 导出登录态。"
...
def inject_bearer_into_state(state: dict[str, Any], bearer_token: str) -> dict[str, Any]:
    ...
    auth = dict(enriched.get("stepfun_auth") or {})
    auth["bearer_token"] = token
    session_id = extract_session_cookie(enriched)
    if session_id:
        auth["session_id"] = session_id
    enriched["stepfun_auth"] = auth

    user_token_json = json.dumps({"value": token, "__version": "0"}, ensure_ascii=False)
    origins = enriched.setdefault("origins", [])
    if not origins:
        origins.append({"origin": "https://chat.stepfun.com", "localStorage": []})

    for origin_data in origins:
        if not isinstance(origin_data, dict):
            continue
        if origin_data.get("origin") not in {None, "https://chat.stepfun.com", "https://www.stepfun.com"}:
            continue
...
def merge_cookie_lists(*groups: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for cookies in groups:
        for cookie in cookies:
            ...
            domain = str(cookie.get("domain") or ".stepfun.com")
