...
BROWSER_MODELS = [
    {"id": "stepfun-chat-web", "object": "model", "owned_by": "stepfun-web"},
]
LOGGER_NAME = "stepfun_openai_proxy"
logger = logging.getLogger(LOGGER_NAME)
_LOG_MAX_CHARS = max(0, int(os.getenv("STEPFUN_LOG_MAX_CHARS", "400")))
...
def _parse_web_mode(value: object) -> str | None:
    ...
    if not isinstance(value, str):
        raise ValueError(f"invalid StepFun web mode: {value!r}")
    normalized = value.strip().lower()
    if normalized in {"", "default"}:
        return None
    aliases = {
...
def resolve_new_chat(
    ...
    *,
    ...
    """Resolve whether browser mode should start a fresh StepFun web session.

    Priority (first match wins):
    1. Request body ``new_chat``
    2. Request body ``metadata.new_chat`` (OpenAI SDK ``extra_body``)
    3. HTTP header ``X-StepFun-New-Chat``
    4. Environment default ``STEPFUN_NEW_CHAT_PER_REQUEST``
    """
...
def resolve_web_mode(
    ...
    *,
    ...
    """Resolve the StepFun web chat mode for browser mode.

    Priority mirrors ``new_chat``:
    1. Request body ``stepfun_mode``
    2. Request body ``metadata.stepfun_mode``
    3. HTTP header ``X-StepFun-Mode``
    4. Environment/backend default
    """
    if "stepfun_mode" in payload:
        parsed = _parse_web_mode(payload["stepfun_mode"])
        if parsed is not None:
            return parsed

    metadata = payload.get("metadata")
    if isinstance(metadata, dict) and "stepfun_mode" in metadata:
        parsed = _parse_web_mode(metadata["stepfun_mode"])
        if parsed is not None:
            return parsed
...
def resolve_deep_thinking(
    ...
    *,
    ...
    """Resolve whether StepFun web deep thinking should be enabled.

    StepFun's inspected web UI does not currently expose a dedicated
    deep-thinking toggle on the homepage. The value is parsed for client
    compatibility, and the browser backend can ignore it when no matching
    control exists.
    """
...
def create_app(
    *,
    ...
    stepfun_base_url: str = DEFAULT_BASE_URL,
    ...
    configure_logging(env_var="STEPFUN_LOG_LEVEL")

    local_api_key = local_api_key if local_api_key is not None else os.getenv("STEPFUN_PROXY_API_KEY", "local-secret")
    stepfun_api_key = stepfun_api_key if stepfun_api_key is not None else os.getenv("STEPFUN_API_KEY")
    stepfun_base_url = os.getenv("STEPFUN_BASE_URL", stepfun_base_url).rstrip("/")
    backend_mode = (backend_mode or os.getenv("STEPFUN_BACKEND", "browser")).lower()
    ...
    app = FastAPI(title="stepfun-openai-proxy", version="0.1.0", lifespan=lifespan)
    ...
    install_console_ingest_middleware(app, proxy_id="stepfun-openai-proxy")
    ...
    async def chat_completions(
        ...
        x_stepfun_new_chat: str | None = Header(default=None, alias="X-StepFun-New-Chat"),
        x_stepfun_mode: str | None = Header(default=None, alias="X-StepFun-Mode"),
        x_stepfun_deep_thinking: str | None = Header(default=None, alias="X-StepFun-Deep-Thinking"),
    ):
        _require_local_key(authorization, local_api_key)
        ...
        if backend_mode == "browser":
            ...
            if browser_backend is None:
                try:
                    from .browser_client import BrowserStepFunClient
                except ImportError:
                    from browser_client import BrowserStepFunClient

                browser_backend = BrowserStepFunClient.from_env()
            new_chat = resolve_new_chat(
                payload,
                header=x_stepfun_new_chat,
                default=browser_backend.new_chat_per_request,
            )
            web_mode = resolve_web_mode(
                payload,
                header=x_stepfun_mode,
                default=getattr(browser_backend, "default_web_mode", "fast"),
            )
            deep_thinking = resolve_deep_thinking(
                payload,
                header=x_stepfun_deep_thinking,
                default=getattr(browser_backend, "default_deep_thinking", False),
            )
            session_id = extract_session_id(payload)
            if web_mode == "deep_research" and not new_chat:
                logger.info(
                    "chat.force_new_chat=true reason=deep_research_no_multiturn session_id=%s",
                    session_id or "-",
                )
                new_chat = True
            logger.info(
            ...
            model = str(payload.get("model") or "stepfun-chat-web")
            logger.info(
        ...
        if not stepfun_api_key:
            return _error(500, "STEPFUN_API_KEY is not configured", "configuration_error")
        ...
        try:
            ...
            return _error(502, f"StepFun upstream request failed: {exc}", "upstream_error")
