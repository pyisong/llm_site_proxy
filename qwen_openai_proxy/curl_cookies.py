...
def parse_curl_cookies(curl_text: str, *, domain: str = ".qwen.ai") -> tuple[list[dict[str, object]], str | None]:
...
def load_auth_from_text(text: str, *, domain: str = ".qwen.ai") -> tuple[list[dict[str, object]], str | None]:
    stripped = text.strip()
...
def load_auth_from_file(path: str | Path, *, domain: str = ".qwen.ai") -> tuple[list[dict[str, object]], str | None]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="从 curl 命令解析 Qwen cookie")
    parser.add_argument("input", nargs="?", help="curl 文件路径；省略则从 stdin 读取")
    parser.add_argument("-o", "--output", default="secrets/qwen_cookies.json", help="输出 JSON 路径")
    parser.add_argument("--domain", default=".qwen.ai", help="cookie domain")
