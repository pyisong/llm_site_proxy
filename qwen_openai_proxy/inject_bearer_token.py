"""向已有 qwen_storage.json 写入 Bearer 令牌。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="向 storage state 注入 Bearer 令牌（可从 DevTools → Network → completions 请求头复制）"
    )
    parser.add_argument(
        "path",
        nargs="?",
        default=os.getenv("QWEN_STORAGE_STATE_FILE", "secrets/qwen_storage.json"),
    )
